...
public class RSASignUtils {
    ...
    public boolean verify(byte[] data, String publicKey, String sign)
        ...
        signature.update(data);
        return signature.verify(Base64.decodeString(sign));
    }
    ...
    public byte[] decryptByPrivateKey(byte[] encryptedData, String privateKey)
        ...
        int blockLength = getKeyBitLength(privateK)/8;
        ...
        while (inputLen - offSet > 0) {
            if (inputLen - offSet > blockLength) {
                cache = cipher.doFinal(encryptedData, offSet, blockLength);
            } else {
    ...
    public byte[] decryptByPublicKey(byte[] encryptedData, String publicKey)
        ...
        int blockLength = getKeyBitLength(publicK)/8;
        Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
    ...
    public byte[] encryptByPublicKey(byte[] data, String publicKey)
            throws Exception {
//    	System.out.println("数据 utf-8:" + new String(data,"UTF-8"));
        byte[] keyBytes = Base64.decodeString(publicKey);
        X509EncodedKeySpec x509KeySpec = new X509EncodedKeySpec(keyBytes);
        KeyFactory keyFactory = KeyFactory.getInstance(KEY_ALGORITHM);
        Key publicK = keyFactory.generatePublic(x509KeySpec);
        int blockLength = getMaxBlockSize(publicK);
    ...
    public byte[] encryptByPrivateKey(byte[] data, String privateKey)
        ...
        KeyFactory keyFactory = KeyFactory.getInstance(KEY_ALGORITHM);
        Key privateK = keyFactory.generatePrivate(pkcs8KeySpec);
        int blockLength = getMaxBlockSize(privateK);
        Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
   ...
   private static int getMaxBlockSize(Key key){
	   return getKeyBitLength(key)/8-11;
   }
