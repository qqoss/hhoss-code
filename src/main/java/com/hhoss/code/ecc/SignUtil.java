...
import static com.hhoss.code.ecc.ECConstant.CURVE;
...
public class SignUtil {
	...
	public static ECPoint recoverPublicPoint(int recId, BigInteger R, BigInteger S, byte[] hash) {
		Assert.verify(recId >= 0, "recId must be positive");
		Assert.verify(hash != null, "message cannot be null");
		Assert.verify(R.signum() >= 0, "r must be positive");
		Assert.verify(S.signum() >= 0, "s must be positive");
	 ...
	 *{@link SignUtil#HALF_CURVE_ORDER}. See <a href="https://github.com/bitcoin/bips/blob/master/bip-0062.mediawiki#Low_S_values_in_signatures">BIP62</a>.
	...
	/**
	 * SignUtil a message with the private key of this key pair.
	 *
	...
	/**
	 * SignUtil a hash with the private key of this key pair.
	 *
