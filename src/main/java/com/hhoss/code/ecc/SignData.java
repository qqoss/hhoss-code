...
public class SignData {
	...
	public byte[] getPublicKey() {
		Assert.verify(r != null && r.length == 32, "r must be 32 bytes");
		Assert.verify(s != null && s.length == 32, "s must be 32 bytes");
		...
		return SignUtil.recoverPublicKeys(header-LOWER_REAL_V,R,S, h);
	}
